    ...
    <main class="flex-1">
        ...
        <div class="p-8 space-y-8">
            <div class="bg-white rounded-3xl shadow-sm p-8 border border-slate-100">
                <h3 class="text-lg font-bold text-slate-800 mb-6"><i
                        class="fa-solid fa-plus-circle text-orange-500 mr-2"></i> Ajouter une réalisation</h3>

                <form action="{{ route('artisan.portfolio.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="md:col-span-1">
                        <label class="block text-xs font-black text-slate-400 uppercase mb-2">Image de
                            réalisation</label>
                    ...
                    <div class="md:col-span-1 flex items-end">
                        <button type="submit"
                            class="w-full py-4 bg-slate-900 text-white font-black rounded-2xl hover:bg-orange-600 transition shadow-lg">
                            Télécharger l'image
                        </button>
                    </div>
                </form>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($portfolios as $work)
                    <div
                        class="group relative bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-xl transition-all duration-300">
                        <img src="{{ asset('storage/' . $work->image_path) }}" alt="{{ $work->title }}"
                            class="w-full h-64 object-cover group-hover:scale-105 transition duration-500">

                        <div
                            class="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-6 flex flex-col justify-end">
                            ...
                            <form action="{{ route('artisan.portfolio.destroy', $work->id) }}" method="POST"
                                ...
                                @method('DELETE')
                                <button type="submit"
                                    class="w-full py-2 bg-red-500/20 hover:bg-red-500 text-red-500 hover:text-white border border-red-500/50 rounded-xl text-xs font-black transition">
